...
export const createCard = async (
  ...
  try {
    const { name, singer, link, list_id } = req.body;
    ...
    const song = await CardModel.create({
      name,
      singer,
      link,
      list_id,
...
export const updateCard = async (
  ...
  try {
    ...
    const newSong = await CardModel.findByIdAndUpdate(
      id,
      {
        name,
        singer,
...
export const deleteCard = async (
  ...
  try {
    ...
    list.songs = list.songs.filter((cardId) => cardId.toString() !== id);
    await list.save();
