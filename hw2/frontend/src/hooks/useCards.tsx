import {
  ...
  import type { PlaylistWindowProps } from "@/components/PlaylistWindow";
  import { getCards, getLists } from "@/utils/client";
  
  type CardContextType = {
    lists: PlaylistWindowProps[];
  ...
  export function CardProvider({ children }: CardProviderProps) {
    ...
    const lists = useMemo(() => {
      ...
      const listMap = rawLists.reduce(
        ...
        {} as Record<string, PlaylistWindowProps>,
      );
      // or you can do for loops
      for (const card of rawCards) {
        listMap[card.list_id].songs.push({
          ...card,
          playlistId: card.list_id,
        });
      }
