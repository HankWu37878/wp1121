...
export async function POST(request: NextRequest) {
  ...
  const { username, content, startDate, endDate } = data as PostActivityRequest;
  ...
  try {
    ...
    const newActivityId = queryResult[0]?.id;


    return NextResponse.json(
      { message: "Activity created successfully", id: newActivityId },
      { status: 200 });
  } catch (error) {
